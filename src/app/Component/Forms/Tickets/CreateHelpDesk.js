...
import WorkNoteHelpdesk from '@/Component/UI/Tickets/WorkNoteHelpdesk';
...
const Create = ({ user_id, createCallback }) => {
  ...
  const [classDisableList, setClassDisableList] = useState([]);
  ...
  const workNoteCallback = (data) => {
    ...
    if (data.result.length > 0) {
      ...
      // record.tag_id === 2 ? setStatusDisableList(['Resolved','Closed']) : setStatusDisableList(['Open'])
  ...
  return (
    <>
      <form onSubmit={submitForm} >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 md:gap-10 mt-4">
              ...
              <div className="mx-10">
                <Form_Item_dropdown
                  ...
                  isShow={true}
                  getValueCallback={getSelectCallback}
                  optionDisabled={classDisableList}
                />
                <Form_Item_dropdown
                  id={'dept_id'}
                  name={'department'}
        ...
        <button
          type="Submit"
          className={`${insertBtn.disabled ? 'bg-green-300' : 'bg-green-500 hover:bg-green-600'} absolute right-28 bottom-3 justify-center rounded-md px-3 py-2 text-sm font-semibold text-white shadow-sm sm:ml-3 sm:w-auto`}
