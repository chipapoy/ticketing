...
const Technician = (props) => {
   ...
   const fetchData = async page => {
      ...
      try {
         ...
         setLoading(true)
         const response = await axios.get(`/api/Technician/getDetails?page=${page}&per_page=${perPage}`)
         setTechnicianArr(response.data.result.data)
   ...
   const handlePerRowsChange = async (newPerPage, page) => {
      ...
      try {
         ...
         const response = await axios.get(`/api/Technician/getDetails?page=${page}&per_page=${newPerPage}`);
         setTechnicianArr(response.data.result.data)
   ...
   const MenuButton = ({menuArr}) => {
      ...
      const DisableItem = (disableSelectedID) => {
         ...
         function Disable(){
            ...
            const url = '/api/Technician/disableData'
   ...
   const Add = () => {
      ...
      const getTechnicianInputCallback = (data) => setNewTechnician(data.target.value)
      const getLocationInputCallback = (data) => setNewLocation(data.target.value)
      ...
      const submitForm = (e) => {
         ...
         if(doesStringExist(data.technician_name, technicianArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }

         const url = '/api/Technician/insertData'
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error){
               ...
               updateToast(`${data.technician_name} has been added!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            }
      ...
      return(
         ...
         >
            ...
            <TextField
               ...
               onChange={getTechnicianInputCallback}
            />
            ...
            <TextField
               ...
               id="outlined-required"
               label="New Location"
               onChange={getLocationInputCallback}
            />
   ...
   const Edit = ( {selectedID} ) => {
      ...
      const [ initTechnician, setInitTechnician ] = useState('')
      ...
      const getTicketDetails = async () => {
         await axios.post(`/api/Technician/getData`,
         {
            id: selectedID
         }, {
         headers: {
         ...
         .then((response) => {
            setSelectedTechnician(response.data.result)
      ...
      const getLocationInputCallback = (data) => setInitLocation(data.target.value)
      const getTechNameInputCallback = (data) => setInitTechnician(data.target.value)

      const submitForm = (e) => {
         e.preventDefault();

         const data = {
            technician_name: initTechnician === '' ? selectedTechnician.technician_name : initTechnician,
            location: initLocation === '' ? selectedTechnician.location : initLocation,
         ...
         if(doesUpdateStringExist(data.id, data.technician_name, technicianArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }
         ...
         const url = '/api/Technician/updateData'
         ...
         axios.post(url, data, {
            ...
            if(!response.data.result.error){
               updateToast(`${data.technician_name} successfully updated!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            }
      ...
      return(
         ...
         Object.keys(selectedTechnician).length === 0 ? null :
         ( 
            <Box
               component="form"      
               onSubmit={submitForm}
            ...
            >
               ...
               <TextField
                  required
                  id="outlined-required"
                  label="Rename Technician"
                  onChange={getTechNameInputCallback}
                  defaultValue={selectedTechnician.technician_name}
   ...
   const Header = () => {
      return (
         <div>
            <Tooltip placement='top' title='Add Technician'>
            <button
               type="button"
               className="
                     relative 
            ...
            <Tooltip placement='top' title='Download CSV'>
               ...
               className="
                  ...
                  ">
                     <CSVLink 
                     data={technicianArr}
                     filename={`Technician list as of ${moment().format('MM-DD-YYYY HH-mm')}`}>
