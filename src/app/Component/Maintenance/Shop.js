...
const Shop = (props) => {
   ...
   const fetchData = async page => {
      ...
      try {
            ...
            setLoading(true)
            const response = await axios.get(`/api/Shop/getDetails?page=${page}&per_page=${perPage}`)
                           
            setShopArr(response.data.result.data);
   ...
   const handlePerRowsChange = async (newPerPage, page) => {
      ...
      try {
         ...
         const response = await axios.get(`/api/Shop/getDetails?page=${page}&per_page=${newPerPage}`);
         setShopArr(response.data.result.data)
   ...
   const Header = () => {
      return (
         <div>
            <Tooltip placement='top' title='Add Shop'>
               <button
                  type="button"
                  className="
                        relative 
            ...
            <Tooltip placement='top' title='Download CSV'>
               <button
                  ...
                  >
                     <CSVLink
                        data={shopArr}
                        filename={`Shop list as of ${moment().format('MM-DD-YYYY HH-mm')}`}
                        >
   ...
   const MenuButton = ({menuArr}) => {
      ...
      const DisableItem = (disableSelectedID) => {
         ...
         function Disable(){
            ...
            const url = '/api/Shop/disableData'
   ...
   const Add = () => {
      ...
      const getNewShop = (data) => setNewShop(data.target.value)
      const getNewShopCode = (data) => setNewShopCode(data.target.value)

      const submitForm = (e) => {
         e.preventDefault();

         const url = `/api/Shop/insertData`
         ...
         if (doesStringWithCodeExist(data.shop_code, data.shop_name, shopArr)){
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error) {
               ...
               updateToast(`${data.shop_name} has been added!`, 'success', 3000, 
               () => {
                  setRefresh(true)
               })
            }
   ...
   const Edit = ( {selectedID} ) => {
      ...
      const [ initShopCode, setInitShopCode ] = useState('')
      ...
      const getTicketDetails = async() => {
         await axios.post(`/api/Shop/getData`,
         {
            id: selectedID
         }, {
            headers: {
            ...
            setSelectedShop(response.data.result)
      ...
      const getShopCallback = (data) => setInitShopName(data.target.value)
      const getShopCodeCallback = (data) => setInitShopCode(data.target.value)
      ...
      const submitForm = (e) => {
         ...
         const data ={
            shop_code: initShopCode === '' ? selectedShop.shop_code : initShopCode,
            shop_name: initShopName === '' ? selectedShop.shop_name : initShopName,
         ...
         if (doesUpdateStringWithCodeExist(data.id, data.shop_code, data.shop_name, shopArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error){
               updateToast(`${data.shop_name} successfully updated!`, 'success', 3000, 
               () => {
                  setRefresh(true)
               })
            } 
      ...
      return (
         Object.keys(selectedShop).length === 0 ? null:
         (
            ...
            <Box 
               ...
               <div>
                  <TextField
                     ...
                     id="outlined-required"
                     label="Shop Code"
                     onChange={getShopCodeCallback}
                     sx={{
                        my: 1
                     }}
                     size='small'
                     defaultValue={selectedShop.shop_code}
                  />
               ...
               <div>
                  <TextField
                     ...
                     id="outlined-required"
                     label="Shop"
                     onChange={getShopCallback}
                     sx={{
                        my: 1
                     }}
                     size='small'
                     defaultValue={selectedShop.shop_name}
                  />
