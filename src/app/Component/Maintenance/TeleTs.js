...
const TeleTs = (props) => {
   ...
   const fetchData = async page => {
      ...
      try {
         ...
         setLoading(true)
         const response = await axios.get(`/api/TeleTs/getDetails?page=${page}&per_page=${perPage}`)
   ...
   const handlePerRowsChange = async (newPerPage, page) => {
      ...
      try {
         ...
         const response = await axios.get(`/api/TeleTs/getDetails?page=${page}&per_page=${newPerPage}`);
         setTeleTsArr(response.data.result.data)
   ...
   const MenuButton = ({menuArr}) => {
      ...
      const DisableItem = (disableSelectedID) => {
         ...
         function Disable(){
            ...
            const url = '/api/TeleTs/disableData'
   ...
   //#region ADD
   const Add = () => {
      const [newTeleTs, setNewTeleTs] = useState('')

      const getInputCallback = (data) => setNewTeleTs(data.target.value)
      ...
      const submitForm = (e) => {
         ...
         if(doesStringExist(data.tele_ts, teleTsArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }

         const url = '/api/TeleTs/insertData'
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error){
               ...
               updateToast(`${data.tele_ts} has been added!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            }
   ...
   const Edit = ( {selectedID} ) => {
      const [ selectedTeleTs, setSelectedTeleTs ] = useState({})
      const [ initTeleTs, setInitTeleTs ] = useState('')
      ...
      const getTicketDetails = async () => {
         await axios.post(`/api/TeleTs/getData`,
         {
            id: selectedID
         }, {
         headers: {
         ...
         .then((response) => {
            ...
            setSelectedTeleTs(response.data.result)
      ...
      const getInputCallback = (data) => setInitTeleTs(data.target.value)

      const submitForm = (e) => {
         e.preventDefault();

         const data = {
            tele_ts: initTeleTs === '' ? selectedTeleTs.tele_ts : initTeleTs,
         ...
         if(doesUpdateStringExist(data.id, data.tele_ts, teleTsArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }
         ...
         const url = '/api/TeleTs/updateData'
         ...
         axios.post(url, data, {
            ...
            if(!response.data.result.error)
            {
               updateToast(`${data.tele_ts} successfully updated!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            }
      ...
      return(
         ...
         Object.keys(selectedTeleTs).length === 0 ? null :
         ( 
            <Box
               component="form"      
               onSubmit={submitForm}
            ...
            >
               ...
               <TextField
                  required
                  id="outlined-required"
                  label="Rename Tele Troubleshoot"
                  onChange={getInputCallback}
                  defaultValue={selectedTeleTs.tele_ts}
   ...
   const Header = () => {
      return (
         <div>
            <Tooltip placement='top' title='Add Tele Troubleshoot'>
            <button
               type="button"
               className="
                     relative 
            ...
            <Tooltip placement='top' title='Download CSV'>
               ...
               className="
                  ...
                  ">
                     <CSVLink 
                     data={teleTsArr}
                     filename={`Tele Troubleshoot list as of ${moment().format('MM-DD-YYYY HH-mm')}`}>
