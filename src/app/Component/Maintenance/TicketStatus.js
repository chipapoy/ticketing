...
const TicketStatus = (props) => {
   const [ticketStatusArr, setTicketStatusArr] = useState([])
   ...
   const fetchData = async page => {
      ...
      try {
         ...
         setLoading(true)
         const response = await axios.get(`/api/Status/getDetails?page=${page}&per_page=${perPage}`)
         setTicketStatusArr(response.data.result.data)
   ...
   const handlePerRowsChange = async (newPerPage, page) => {
      ...
      try {
         ...
         const response = await axios.get(`/api/Status/getDetails?page=${page}&per_page=${newPerPage}`);
         setTicketStatusArr(response.data.result.data)
   ...
   const MenuButton = ({menuArr}) => {
      ...
      const DisableItem = (disableSelectedID) => {
         ...
         function Disable(){
            ...
            const url = '/api/Status/disableData'
   ...
   const Add = () => {
      ...
      const getInputCallback = (data) => setNewStatus(data.target.value)
      ...
      const submitForm = (e) => {
         ...
         if(doesStringExist(data.status, ticketStatusArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }

         const url = '/api/Status/insertData'
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error){
               ...
               updateToast(`${data.status} has been added!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            }
   ...
   const Edit = ( {selectedID} ) => {
      ...
      const getTicketDetails = async () => {
         await axios.post(`/api/Status/getData`,
         {
            id: selectedID
         }, {
         headers: {
         ...
         .then((response) => {
            setSelectedStatus(response.data.result)
      ...
      const getInputCallback = (data) => setInitStatus(data.target.value)

      const submitForm = (e) => {
         e.preventDefault();

         const data = {
            status: initStatus === '' ? selectedStatus.status : initStatus,
         ...
         if (doesUpdateStringExist(data.id, data.status, ticketStatusArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }
         ...
         const url = '/api/Status/updateData'
         ...
         axios.post(url, data, {
            ...
            if(!response.data.result.error){
               updateToast(`${data.status} successfully updated!`, 'success',  3000, 
               () => { 
                  setRefresh(true)
               })
            }
      ...
      return(
         ...
         Object.keys(selectedStatus).length === 0 ? null :
         ( 
            <Box
               component="form"      
               onSubmit={submitForm}
            ...
            >
               <div>
                  <TextField
                     ...
                     label="Rename Status"
                     onChange={getInputCallback}
                     defaultValue={selectedStatus.status}
         ...
         <div>
            ...
            <Tooltip placement='top' title='Download CSV'>
               ...
               className="
                  ...
                  ">
                     <CSVLink
                     data={ticketStatusArr}
                     filename={`Ticket Status list as of ${moment().format('MM-DD-YYYY HH-mm')}`}>
