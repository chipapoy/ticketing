...
const Department = (props) => {
   ...
   const fetchData = async page => {
      ...
      try {
            ...
            setLoading(true)
            const response = await axios.get(`/api/Department/getDetails?page=${page}&per_page=${perPage}`)
                           
            setDepartmentArr(response.data.result.data);
      ...
      const handlePerRowsChange = async (newPerPage, page) => {
         ...
         try {
               ...
               const response = await axios.get(`/api/Department/getDetails?page=${page}&per_page=${newPerPage}`);
               setDepartmentArr(response.data.result.data)
   ...
   const MenuButton = ({menuArr}) => {
      ...
      const DisableItem = (disableSelectedID) => {
         ...
         function Disable(){
            ...
            const url = '/api/Department/disableData'
   ...
   const Add = () => {
      ...
      const getNewDepartmentName = (data) => setNewDeptName(data.target.value)
      const getNewDeptCode = (data) => setNewDeptCode(data.target.value)

      const submitForm = (e) => {
         e.preventDefault();

         const url = `/api/Department/insertData`
         ...
         if(doesStringWithCodeExist(data.dept_code, data.department, departmentArr)){
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error) {
               ...
               updateToast(`${data.department} has been added!`, 'success', 3000, () => 
               {
                  setRefresh(true)
               })
            }
   ...
   const Edit = ( {selectedID} ) => {
      ...
      const [ initDepartmentName, setInitDepartmentName ] = useState('')
      const [ initDeptCode, setInitDeptCode ] = useState('')
      ...
      const getTicketDetails = async() => {
         await axios.post(`/api/Department/getData`,
         {
            id: selectedID
         }, {
            headers: {
            ...
            setSelectedDepartment(response.data.result)
      ...
      const getDeptCallback = (data) => setInitDepartmentName(data.target.value)
      const getDeptCodeCallback = (data) => setInitDeptCode(data.target.value)
      ...
      const submitForm = (e) => {
         ...
         const data ={
            dept_code: initDeptCode === '' ? selectedDepartment.dept_code : initDeptCode,
            department: initDepartmentName === '' ? selectedDepartment.department : initDepartmentName,
         ...
         if(doesUpdateStringWithCodeExist(data.id, data.dept_code, data.department, departmentArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }
         ...
         const url = '/api/Department/updateData'
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error){
               updateToast(`${data.department} successfully updated!`, 'success', 3000, 
               () => {
                  setRefresh(true)
               })
            } 
      ...
      return (
         Object.keys(selectedDepartment).length === 0 ? null:
         (
            <Box 
               component="form"
               onSubmit={submitForm}
         ...
         <div>
            ...
            <Tooltip placement='top' title='Download CSV'>
               ...
               className="
                  ...
                  <CSVLink
                     data={departmentArr}
                     filename={`Department list as of ${moment().format('MM-DD-YYYY HH-mm')}`}>
