...
const Class = (props) => {
   ...
   const fetchData = async page => {
      ...
      try {
         ...
         setLoading(true)
         const response = await axios.get(`/api/Classification/getDetails?page=${page}&per_page=${perPage}`)
         setClassArr(response.data.result.data)
   ...
   const handlePerRowsChange = async (newPerPage, page) => {
      ...
      try {
         ...
         const response = await axios.get(`/api/Classification/getDetails?page=${page}&per_page=${newPerPage}`);
         setClassArr(response.data.result.data)
   ...
   const MenuButton = ({menuArr}) => {
      ...
      const DisableItem = (disableSelectedID) => {
         ...
         function Disable(){
            ...
            const url = '/api/Classification/disableData'
   ...
   const Add = () => {
      ...
      const getInputCallback = (data) => setNewClass(data.target.value)
      ...
      const submitForm = (e) => {
         ...
         if(doesStringExist(data.classification, classArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }

         const url = '/api/Classification/insertData'
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error){
               ...
               updateToast(`${data.classification} has been added!`, 'success', 3000, () => {
                  setRefresh(true)
               })
            }
            else {
   ...
   const Edit = ( {selectedID} ) => {
      ...
      const getTicketDetails = async () => {
         await axios.post(`/api/Classification/getData`,
         {
            id: selectedID
         }, {
         headers: {
         ...
         .then((response) => {
            setSelectedClass(response.data.result)
      ...
      const getInputCallback = (data) => setInitClass(data.target.value)

      const submitForm = (e) => {
         e.preventDefault();

         const data = {
            classification: initClass === '' ? selectedClass.classification : initClass,
         ...
         if(doesUpdateStringExist(data.id, data.classification, classArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }
         ...
         const url = '/api/Classification/updateData'
         ...
         axios.post(url, data, {
            ...
            if(!response.data.result.error){
               updateToast(`${data.classification} successfully updated!`, 'success', 3000, () => {
                  setRefresh(true)
               })
            }
            else 
      ...
      return(
         ...
         Object.keys(selectedClass).length === 0 ? null :
         ( 
            <Box
               component="form"      
               onSubmit={submitForm}
            ...
            >
               ...
               <TextField
                  required
                  id="outlined-required"
                  label="Rename Classification"
                  onChange={getInputCallback}
                  defaultValue={selectedClass.classification}
   ...
   const Header = () => {
      return (
         <div>
            <Tooltip placement='top' title='Add Class'>
            <button
               type="button"
               className="
                     relative 
            ...
            <Tooltip placement='top' title='Download CSV'>
               ...
               className="
                  ...
                  <CSVLink
                     data={classArr}
                     filename={`Classification list as of ${moment().format('MM-DD-YYYY HH-mm')}`}
                  >
