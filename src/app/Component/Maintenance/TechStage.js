...
const TechStage = (props) => {
   const [techStageArr, setTechStageArr] = useState([])
   ...
   const fetchData = async page => {
      ...
      try {
            ...
            setLoading(true)
            const response = await axios.get(`/api/Stage2/getDetails?page=${page}&per_page=${perPage}`)
                           
            setTechStageArr(response.data.result.data);
      ...
      const handlePerRowsChange = async (newPerPage, page) => {
         ...
         try {
               ...
               const response = await axios.get(`/api/Stage2/getDetails?page=${page}&per_page=${newPerPage}`);
               setTechStageArr(response.data.result.data)
   ...
   const MenuButton = ({menuArr}) => {
      ...
      const DisableItem = (disableSelectedID) => {
         ...
         function Disable(){
            ...
            const url = '/api/Stage2/disableData'
   ...
   //#region ADD
   const Add = () => {
      const [newStageCode, setStageCode] = useState('')
      const [newStageName, setNewStageName] = useState('')

      const getNewStageName = (data) => setNewStageName(data.target.value)
      const getNewStageCode = (data) => setStageCode(data.target.value)

      const submitForm = (e) => {
         e.preventDefault();

         const url = `/api/Stage2/insertData`
         ...
         if(doesStringWithCodeExist(data.stage_code, data.stage, techStageArr)){
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error) {
               ...
               updateToast(`${data.stage} has been added!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            }
         ...
         <Box
            ...
            <div>
               <TextField
                  ...
                  label="New Stage Code"
                  onChange={getNewStageCode}
                  sx={{
                     my: 1
                  }}
                  size='small'
   ...
   const Edit = ( {selectedID} ) => {
      const [ selectedTechStage, setselectedTechStage ] = useState({})
      const [ initStageName, setInitStageName ] = useState('')
      const [ initStageCode, setInitStageCode ] = useState('')
      ...
      const getTicketDetails = async() => {
         await axios.post(`/api/Stage2/getData`,
         {
            id: selectedID
         }, {
            headers: {
            ...
            setselectedTechStage(response.data.result)
      ...
      const getDeptCallback = (data) => setInitStageName(data.target.value)
      const getDeptCodeCallback = (data) => setInitStageCode(data.target.value)
      ...
      const submitForm = (e) => {
         ...
         const data ={
            stage_code: initStageCode === '' ? selectedTechStage.stage_code : initStageCode,
            stage: initStageName === '' ? selectedTechStage.stage : initStageName,
         ...
         if(doesUpdateStringWithCodeExist(data.id, data.stage_code, data.stage, techStageArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }
         ...
         const url = '/api/Stage2/updateData'
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error){
               updateToast(`${data.stage} information updated!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            } else {
      ...
      return (
         Object.keys(selectedTechStage).length === 0 ? null:
         (
            ...
            <Box 
               ...
               <div>
                  <TextField
                     ...
                     onChange={getDeptCodeCallback}
                     sx={{
                        my: 1
                     }}
                     size='small'
                     defaultValue={selectedTechStage.stage_code}
                  />
               ...
               <div>
                  <TextField
                     ...
                     id="outlined-required"
                     label="Department"
                     onChange={getDeptCallback}
                     sx={{
                        my: 1
                     }}
                     size='small'
                     defaultValue={selectedTechStage.stage}
                  />
   ...
   const Header = () => {
      return (
         <div>
            <Tooltip placement='top' title='Add Tech Stage'>
               <button
                  type="button"
                  className="
                        relative 
            ...
            <Tooltip placement='top' title='Download CSV'>
               ...
               className="
                  ...
                  <CSVLink 
                  data={techStageArr}
                  filename={`Tech stage list as of ${moment().format("MM-DD-YYYY HH-mm")}`}>
