...
const ShopLocation = (props) => {
   const [shopLocArr, setShopLocArr] = useState([])
   ...
   useEffect(() => {
      ...
      getCluster().then((response) => {
         setClusterArr(response.data.result);
      })
   ...
   const fetchData = async page => {
		...
		try {
			...
			setLoading(true)
			const response = await axios.get(`/api/ShopLocation/getDetails?page=${page}&per_page=${perPage}`)
						
			setShopLocArr(response.data.result.data);
	...
	const handlePerRowsChange = async (newPerPage, page) => {
		...
		try {
			...
			const response = await axios.get(`/api/ShopLocation/getDetails?page=${page}&per_page=${newPerPage}`);
			setShopLocArr(response.data.result.data)
   ...
   const MenuButton = ({menuArr}) => {
      ...
      const DisableItem = (disableSelectedID) => {
         ...
         function Disable(){
            ...
            const url = '/api/ShopLocation/disableData'
   ...
   //#region ADD
   const Add = () => {
      const [clusterSelectedID, setClusterSelectedID] = useState(0)
      const [newLocationName, setNewLocationName] = useState('')
      const [newAreaCode, setNewAreaCode] = useState('')

      const getClusterSelectCallback = (data) => {
         setClusterSelectedID(data.value.id)
      }

      const getNewLocationName = (data) => {
         setNewLocationName(data.target.value)
      }

      const getNewAreaCode = (data) => {
         setNewAreaCode(data.target.value)
      }
      const submitForm = (e) => {
         e.preventDefault();

         const url = `/api/ShopLocation/insertData`

         const data = {
            area_code: newAreaCode,
            area_location: newLocationName,
            cluster_id: clusterSelectedID,
            added_by: userDataID,
            added_date: moment().format('YYYY-MM-DD HH:mm:ss')
         }

         if(doesStringWithCodeExist(data.area_code, data.area_location, shopLocArr)){
         ...
         axios.post(url, data, {
				...
				if (!response.data.result.error) 
            {
               ...
               updateToast(`${data.area_location} has been added!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            }
            ...
            autoComplete="off"
         	>
               ...
               <Form_Item_dropdown 
                  ...
                  label={'Cluster'}
                  variant={'outlined'}
                  listOfItems={clusterArr}
                  isRequired={true}
                  isDisabled={false}
                  getValueCallback={getClusterSelectCallback}
               />
               <button
                  type="Submit"
   ...
   const Edit = ({selectedID}) => {
      ...
      const [ initShopLocName, setInitShopLocName ] = useState('')
      const [ initAreaCode, setInitAreaCode ] = useState('')
      const [ initClusterID, setInitClusterID ] = useState(0)
      ...
      const getTicketDetails = async() => {
         await axios.post(`/api/ShopLocation/getData`,
         {
            id: selectedID
         }, {
            headers: {
            ...
            setSelectedShopLocation(response.data.result)
      ...
      const getNameCallback = (data) => setInitShopLocName(data.target.value)
      const getAreaCodeCallback = (data) => setInitAreaCode(data.target.value)
      const getClusterCallback = (data) => setInitClusterID(data.value.id)
      ...
      const submitForm = (e) => {
         ...
         const data ={
            area_code: initAreaCode === '' ? selectedShopLocation.area_code : initAreaCode,
            area_location: initShopLocName === '' ? selectedShopLocation.location_name : initShopLocName,
            cluster_id: initClusterID === 0 ? selectedShopLocation.cluster_id : initClusterID,
         ...
         if (doesUpdateStringWithCodeExist(data.id, data.area_code, data.area_location, shopLocArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }
         ...
         const url = '/api/ShopLocation/updateData'
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error){
               updateToast(`${data.area_location} successfully updated!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            } else {
      ...
      return (
         Object.keys(selectedShopLocation).length === 0 ? null:
         (
            ...
            <Box 
               ...
               autoComplete="Off"
               >
                  {console.log(selectedShopLocation)}
               <div>
                  <TextField
                     required
                     id="outlined-required"
                     label="Area Code"
                     onChange={getAreaCodeCallback}
                     sx={{
                        my: 1
                     }}
                     size='small'
                     defaultValue={selectedShopLocation.area_code}
                  />
               ...
               <div>
                  <TextField
                     ...
                     size='small'
                     defaultValue={selectedShopLocation.location_name}
                  />
               ...
               <Form_Item_dropdown 
                  ...
                  isRequired={true}
                  isDisabled={false}
                  getValueCallback={getClusterCallback}
                  selectedValue={{
                     id: selectedShopLocation.cluster_id,
                     name: selectedShopLocation.cluster_name
                  }}
               />
   ...
   const Header = () => {
      return (
         <div>
            <Tooltip placement='top' title='Add Shop Location'>
               <button
					type="button"
					className="
						relative 
            ...
            <Tooltip placement='top' title='Download CSV'>
               ...
               className="
                  ...
                  ">
                     <CSVLink
                        data={shopLocArr}
                        filename={`Shop location list as of ${moment().format('MM-DD-YYYY HH-mm')}`}>
