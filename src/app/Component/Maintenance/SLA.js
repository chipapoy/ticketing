...
const SLA = (props) => {
   ...
   const fetchData = async page => {
      ...
      try {
         ...
         setLoading(true)
         const response = await axios.get(`/api/SLA/getDetails?page=${page}&per_page=${perPage}`)
         setSLAArr(response.data.result.data)
   ...
   const handlePerRowsChange = async (newPerPage, page) => {
      ...
      try {
         ...
         const response = await axios.get(`/api/SLA/getDetails?page=${page}&per_page=${newPerPage}`);
         setSLAArr(response.data.result.data)
   ...
   const Add = () => {
      ...
      const getInputCallback = (data) => setNewSLA(data.target.value)
      ...
      const submitForm = (e) => {
         ...
         if (doesStringExist(data.sla, slaArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }

         const url = '/api/SLA/insertData'
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error){
               ...
               updateToast(`${data.sla} has been added!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            }
   ...
   const Edit = ( {selectedID} ) => {
      ...
      const [ initSLA, setInitSLA ] = useState('')
      ...
      const getTicketDetails = async () => {
         await axios.post(`/api/SLA/getData`,
         {
            id: selectedID
         }, {
         headers: {
         ...
         .then((response) => {
            setSelectedSLA(response.data.result)
      ...
      const getInputCallback = (data) => setInitSLA(data.target.value)

      const submitForm = (e) => {
         e.preventDefault();

         const data = {
            sla: initSLA === '' ? selectedSLA.sla : initSLA,
         ...
         if(doesUpdateStringExist(data.id, data.sla, slaArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }
         ...
         axios.post(url, data, {
            ...
            if(!response.data.result.error){
               updateToast( `${data.sla} has been updated!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            }
      ...
      return(
         ...
         Object.keys(selectedSLA).length === 0 ? null :
         ( 
            <Box
               component="form"      
               onSubmit={submitForm}
   ...
   const Header = () => {
      return (
         <div>
            <Tooltip placement='top' title='Add SLA'>
            <button
               type="button"
               className="
                     relative 
            ...
            <Tooltip placement='top' title='Download CSV'>
               ...
               className="
                  ...
                  ">
                     <CSVLink
                        data={slaArr}
                        filename={`SLA list as of ${moment().format('MM-DD-YYYY HH-mm')}`}>
