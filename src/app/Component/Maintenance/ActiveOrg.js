...
const ActiveOrg = (props) => {
   ...
   const fetchData = async page => {
      ...
      try {
            ...
            setLoading(true)
            const response = await axios.get(`/api/ActiveOrg/getDetails?page=${page}&per_page=${perPage}`)
   ...
   const handlePerRowsChange = async (newPerPage, page) => {
      ...
      try {
         ...
         const response = await axios.get(`/api/ActiveOrg/getDetails?page=${page}&per_page=${newPerPage}`);
         setActiveOrgArr(response.data.result.data)
   ...
   const Header = () => {
      return (
         <div>
            <Tooltip placement='top' title='Add Active Org'>
               <button
                  type="button"
                  className="
                        relative 
            ...
            <Tooltip placement='top' title='Download CSV'>
               <button
                  ...
                  >
                     <CSVLink
                        data={activeOrgArr}
                        filename={`Active Org list as of ${moment().format('MM-DD-YYYY HH-mm')}`}
                        >
   ...
   const MenuButton = ({menuArr}) => {
      ...
      const DisableItem = (disableSelectedID) => {
         ...
         function Disable(){
            ...
            const url = '/api/ActiveOrg/disableData'
   ...
   const Add = () => {
      ...
      const [newActiveOrg, setNewActiveOrg] = useState('')

      const getNewActiveOrg = (data) => setNewActiveOrg(data.target.value)
      const getNewOrgCode = (data) => setNewOrgCode(data.target.value)

      const submitForm = (e) => {
         e.preventDefault();

         const url = `/api/ActiveOrg/insertData`

         const data = {
            org_code: newOrgCode,
            active_org: newActiveOrg,
            added_by: userDataID,
            added_date: moment().format('YYYY-MM-DD HH:mm:ss')
         }

         if (doesStringWithCodeExist(data.org_code, data.active_org, activeOrgArr)){
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error) {
               ...
               updateToast(`${data.active_org} has been added!`, 'success', 3000, 
               () => {
                  setRefresh(true)
               })
            }
            ...
            autoComplete="off"
               ...
               <div>
                  <TextField
                     required
                     id="outlined-required"
                     label="New Active Org"
                     onChange={getNewActiveOrg}
                     sx={{
                        my: 1
                     }}
                     size='small'
   ...
   const Edit = ( {selectedID} ) => {
      const [ selectedActiveOrg, setSelectedActiveOrg ] = useState({})
      const [ initActiveOrgName, setInitActiveOrgName ] = useState('')
      const [ initOrgCode, setInitOrgCode ] = useState('')
      ...
      const getTicketDetails = async() => {
         await axios.post(`/api/ActiveOrg/getData`,
         {
            id: selectedID
         }, {
            headers: {
            ...
            setSelectedActiveOrg(response.data.result)
      ...
      const getActiveOrgCallback = (data) => setInitActiveOrgName(data.target.value)
      const getOrgCodeCallback = (data) => setInitOrgCode(data.target.value)
      ...
      const submitForm = (e) => {
         ...
         const data ={
            org_code: initOrgCode === '' ? selectedActiveOrg.org_code : initOrgCode,
            active_org: initActiveOrgName === '' ? selectedActiveOrg.active_org : initActiveOrgName,
         ...
         if (doesUpdateStringWithCodeExist(data.id, data.org_code, data.active_org, activeOrgArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }
         ...
         const url = '/api/ActiveOrg/updateData'
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error){
               updateToast(`${data.active_org} successfully updated!`, 'success', 3000, 
               () => {
                  setRefresh(true)
               })
            } 
      ...
      return (
         Object.keys(selectedActiveOrg).length === 0 ? null:
         (
            ...
            <Box 
               ...
               <div>
                  <TextField
                     required
                     id="outlined-required"
                     label="Active Org Code"
                     onChange={getOrgCodeCallback}
                     sx={{
                        my: 1
                     }}
                     size='small'
                     defaultValue={selectedActiveOrg.org_code}
                  />
               ...
               <div>
                  <TextField
                     ...
                     id="outlined-required"
                     label="Active Org"
                     onChange={getActiveOrgCallback}
                     sx={{
                        my: 1
                     }}
                     size='small'
                     defaultValue={selectedActiveOrg.active_org}
                  />
