...
const Role = (props) => {
   ...
   const fetchData = async page => {
      ...
      try {
         ...
         setLoading(true)
         const response = await axios.get(`/api/Role/getDetails?page=${page}&per_page=${perPage}`)
         setRoleArr(response.data.result.data)
   ...
   const handlePerRowsChange = async (newPerPage, page) => {
      ...
      try {
         ...
         const response = await axios.get(`/api/Role/getDetails?page=${page}&per_page=${newPerPage}`);
         setRoleArr(response.data.result.data)
   ...
   const MenuButton = ({menuArr}) => {
      ...
      const DisableItem = (disableSelectedID) => {
         ...
         function Disable(){
            ...
            const url = '/api/Role/disableData'
   ...
   const Add = () => {
      ...
      const getInputCallback = (data) => setNewRole(data.target.value)
      ...
      const submitForm = (e) => {
         ...
         if (doesStringExist(data.role, roleArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }

         const url = '/api/Role/insertData'
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error){
               ...
               updateToast(`${data.role} has been added!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            }
   ...
   const Edit = ( {selectedID} ) => {
      ...
      const getTicketDetails = async () => {
         await axios.post(`/api/Role/getData`,
         {
            id: selectedID
         }, {
         headers: {
         ...
         .then((response) => {
            setSelectedRole(response.data.result)
      ...
      const getInputCallback = (data) => setInitRole(data.target.value)

      const submitForm = (e) => {
         e.preventDefault();

         const data = {
            role: initRole === '' ? selectedRole.role : initRole,
         ...
         if(doesUpdateStringExist(data.id, data.role, roleArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }
         ...
         axios.post(url, data, {
            ...
            if(!response.data.result.error){
               updateToast(`${data.role} successfully updated!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            }
      ...
      return(
         ...
         Object.keys(selectedRole).length === 0 ? null :
         ( 
            <Box
               component="form"      
               onSubmit={submitForm}
   ...
   const Header = () => {
      return (
         <div>
            <Tooltip placement='top' title='Add Role'>
            <button
               type="button"
               className="
                     relative 
            ...
            <Tooltip placement='top' title='Download CSV'>
               ...
               className="
                  ...
                  ">
                     <CSVLink 
                     data={roleArr}
                     filename={`Role list as of ${moment().format('MM-DD-YYYY HH-mm')}`}>
