...
const Priority = (props) => {
   ...
   const fetchData = async page => {
      ...
      try {
         ...
         setLoading(true)
         const response = await axios.get(`/api/Priority/getDetails?page=${page}&per_page=${perPage}`)
   ...
   const handlePerRowsChange = async (newPerPage, page) => {
      ...
      try {
         ...
         const response = await axios.get(`/api/Priority/getDetails?page=${page}&per_page=${newPerPage}`);
         setPriorityArr(response.data.result.data)
   ...
   const MenuButton = ({menuArr}) => {
      ...
      const DisableItem = (disableSelectedID) => {
         ...
         function Disable(){
            ...
            const url = '/api/Priority/disableData'
   ...
   const Add = () => {
      ...
      const getInputCallback = (data) => setNewPriority(data.target.value)
      ...
      const submitForm = (e) => {
         ...
         if(doesStringExist(data.priority, priorityArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }

         const url = '/api/Priority/insertData'
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error){
               ...
               updateToast(`${data.priority} has been added!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            }
   ...
   const Edit = ( {selectedID} ) => {
      ...
      const [ initPriority, setInitPriority ] = useState('')
      ...
      const getTicketDetails = async () => {
         await axios.post(`/api/Priority/getData`,
         {
            id: selectedID
         }, {
         headers: {
         ...
         .then((response) => {
            setSelectedPriority(response.data.result)
      ...
      const getInputCallback = (data) => setInitPriority(data.target.value)

      const submitForm = (e) => {
         e.preventDefault();

         const data = {
            priority: initPriority === '' ? selectedPriority.priority : initPriority,
         ...
         if(doesUpdateStringExist(data.id, data.priority, priorityArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }
         ...
         const url = '/api/Priority/updateData'
         ...
         axios.post(url, data, {
            ...
            if(!response.data.result.error){
               updateToast(`${data.priority} has been updated!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            }
      ...
      return(
         ...
         Object.keys(selectedPriority).length === 0 ? null :
         ( 
            <Box
               component="form"      
               onSubmit={submitForm}
            ...
            >
               <div>
                  <TextField
                     required
                     id="outlined-required"
                     label="Rename Priority"
                     onChange={getInputCallback}
                     defaultValue={selectedPriority.priority}
   ...
   const Header = () => {
      return (
         <div>
            <Tooltip placement='top' title='Add Priority Level'>
            <button
               type="button"
               className="
                     relative 
            ...
            <Tooltip placement='top' title='Download CSV'>
               ...
               className="
                  ...
                  ">
                     <CSVLink
                        data={priorityArr}
                        filename={`Priority list as of ${moment().format('MM-DD-YYYY HH-mm')}`}>
