...
const Tagging = (props) => {
   ...
   const fetchData = async page => {
      ...
      try {
         ...
         setLoading(true)
         const response = await axios.get(`/api/Tagging/getDetails?page=${page}&per_page=${perPage}`)
         setTaggingArr(response.data.result.data)
   ...
   const handlePerRowsChange = async (newPerPage, page) => {
      ...
      try {
         ...
         const response = await axios.get(`/api/Tagging/getDetails?page=${page}&per_page=${newPerPage}`);
         setTaggingArr(response.data.result.data)
   ...
   const MenuButton = ({menuArr}) => {
      ...
      const DisableItem = (disableSelectedID) => {
         ...
         function Disable(){
            ...
            const url = '/api/Tagging/disableData'
   ...
   const Add = () => {
      ...
      const getInputCallback = (data) => setNewTagging(data.target.value)
      ...
      const submitForm = (e) => {
         ...
         if(doesStringExist(data.tagging, taggingArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }

         const url = '/api/Tagging/insertData'
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error){
               ...
               updateToast(`${data.tagging} has been added!`, 'success', 3000,
               () => { 
                  setRefresh(true)
               })
            }
   ...
   const Edit = ( {selectedID} ) => {
      ...
      const [ initTagging, setInitTagging ] = useState('')
      ...
      const getTicketDetails = async () => {
         await axios.post(`/api/Tagging/getData`,
         {
            id: selectedID
         }, {
         headers: {
         ...
         .then((response) => {
            setSelectedTagging(response.data.result)
      ...
      const getInputCallback = (data) => setInitTagging(data.target.value)

      const submitForm = (e) => {
         e.preventDefault();

         const data = {
            tagging: initTagging === '' ? selectedTagging.tagging : initTagging,
         ...
         if(doesUpdateStringExist(data.id, data.tagging, taggingArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }
         ...
         const url = '/api/Tagging/updateData'
         ...
         axios.post(url, data, {
            ...
            if(!response.data.result.error){
               updateToast(`${data.tagging} successfully updated!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            }
      ...
      return(
         ...
         Object.keys(selectedTagging).length === 0 ? null :
         ( 
            <Box
               component="form"      
               onSubmit={submitForm}
            ...
            >
               ...
               <TextField
                  required
                  id="outlined-required"
                  label="Rename Tagging"
                  onChange={getInputCallback}
                  defaultValue={selectedTagging.tagging}
   ...
   const Header = () => {
      return (
         <div>
            <Tooltip placement='top' title='Add Tagging'>
            <button
               type="button"
               className="
                     relative 
            ...
            <Tooltip placement='top' title='Download CSV'>
               ...
               className="
                  ...
                  ">
                     <CSVLink 
                     data={taggingArr}
                     filename={`Tagging list as of ${moment().format('MM-DD-YYYY HH-mm')}`}>
