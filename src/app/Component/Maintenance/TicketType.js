...
const TicketType = (props) => {
   ...
   //#region DATA TABLE
      ...
      const fetchData = async page => {
         ...
         try {
            ...
            setLoading(true)
            const response = await axios.get(`/api/TicketType/getDetails?page=${page}&per_page=${perPage}`)
      ...
      const handlePerRowsChange = async (newPerPage, page) => {
         ...
         try {
            ...
            const response = await axios.get(`/api/TicketType/getDetails?page=${page}&per_page=${newPerPage}`);
            setTicketTypeArr(response.data.result.data)
   ...
   const MenuButton = ({menuArr}) => {
      ...
      const DisableItem = (disableSelectedID) => {
         ...
         function Disable(){
            ...
            const url = '/api/TicketType/disableData'
   ...
   const Add = () => {
      ...
      const getInputCallback = (data) => {
         setNewTicketTypeName(data.target.value)
      }
      ...
      const submitForm = (e) => {
         ...
         const data = {
            ticket_type: newTicketTypeName,
            added_by: userDataID,
            added_date: moment().format('YYYY-MM-DD HH:mm:ss')
         }

         if(doesStringExist(data.ticket_type, ticketTypeArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }

         const url = '/api/TicketType/insertData'
         ...
         axios.post(url, data, {
            ...
            if (!response.data.result.error){
               ...
               updateToast(`${data.ticket_type} has been added!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            }
   ...
   const Edit = ( {selectedID} ) => {
      ...
      const [ initialTicketType, setInitTicketType ] = useState('')
      ...
      const getTicketDetails = async () => {
         await axios.post(`/api/TicketType/getData`,
         {
            id: selectedID
         }, {
         headers: {
         ...
         .then((response) => {
            setSelectedTicketType(response.data.result)
            setInitTicketType(response.data.result.ticket_type)
      ...
      const getInputCallback = (data) => {
         setInitTicketType(data.target.value)
      }
      ...
      const submitForm = (e) => {
         ...
         const data = {
            ticket_type: initialTicketType === '' ? selectedTicketType.ticket_type: initialTicketType,
         ...
         if(doesUpdateStringExist(data.id, data.ticket_type, ticketTypeArr)){
            updateToast('item already exists!', 'error', 1000)
            return
         }
         ...
         const url = '/api/TicketType/updateData'
         axios.post(url, data, {
            headers: {
               'Content-Type': 'multipart/form-data'
            }
            ...
            if(!response.data.result.error){
               updateToast(`${data.ticket_type} successfully updated!`, 'success', 3000, 
               () => { 
                  setRefresh(true)
               })
            }
      ...
      return(  
         Object.keys(selectedTicketType).length === 0 ? null :
         ( 
            <Box
               component="form"      
               autoComplete="off"
            ...
            >
               <div>
                  <TextField
                     required
                     id="outlined-required"
                     label="Rename Ticket Type"
                     onChange={getInputCallback}
                     defaultValue={selectedTicketType.ticket_type}
   ...
   const Header = () => {
      return (
         <div>
            <Tooltip placement='top' title='Add Ticket type'>
            <button
               type="button"
               className="
                     relative 
            ...
            <Tooltip placement='top' title='Download CSV'>  
               ...
               className="
                  ...
                  ">
                     <CSVLink
                     data={ticketTypeArr}
                     filename={`Ticket type list as of ${moment().format('MM-DD-YYYY HH-mm')}`}>
