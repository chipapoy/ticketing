...
import UploadFile from '@/Component/Forms/Tickets/UploadFile';
...
const WorkNote = (props) => {
  ...
  const [form, setForm] = useState({
    ...
    assigned_tech_id: props?.assigned_tech_id,
    tagging: null,
    reso_id: null,
    resolution: null,
    work_note: "",
  ...
  const [disableOnReso, setDisabledOnReso] = useState(true)
    ...
    .then((response) => {
      ...
      if (response.data.result.length > 0) {
        ...
        if(props.status_id != 2){
          if(response.data.result[0].tag_id != 4){

            if(response.data.result[0].user_id == props.user_id){
  ...
  const handleStartTask = () => {
    ...
    axios.post(url, {...form, timestamp: timestamp, added_date: timestamp}, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    })
    .then(response => {
      ...
      if (response.status == 200) {
        ...
        axios.post('/api/TicketList/updateTechData', {...form, timestamp: timestamp}, {
          headers: {
            'Content-Type': 'multipart/form-data'
          }
        })
      ...
      <div className="mx-10">
        ...
        <div className="w-auto mb-2">
          <FormControl >
            ...
            >
              {
                taggingArr?.map((item) => (
                  <FormControlLabel key={item.id} value={item.id} control={<Radio />} label={item.name} disabled={disableForm} />
                ))
              }
            </RadioGroup>
          </FormControl>
        ...
        <div className="w-auto mb-2">
          <Autocomplete
            ...
            disabled={disableForm ? disableForm : disableOnReso}
            renderInput={(params) => (
              <TextField
                {...params}
                label={'Resolution'}
        ...
        <div className="w-auto mb-2 ">
          ...
          <UploadFile fileDataCallback={fileDataCallback} />
        </div>
