...
const WorkNote = (props) => {
  ...
  const [tagOk, setTagOk] = useState(props.tagging != undefined ? 1 : 0)
  ...
  useEffect(() => {
    ...
    setCountOk(tagOk + noteOk)
    if (taggingArr.length === 0) {
      getHelpdeskTagging().then((response) => {
        setTaggingArr(response.data.result)
      });
    }
    countOk == 2 ? setDisableNoteSubmit(false) : setDisableNoteSubmit(true)

    if(props.tagging != undefined){
      setForm(prev => (
        {
          ...prev,
  ...
  return (
    <>
      ...
      <div className="mx-10">
        ...
        <div className="w-auto mb-2">
          <FormControl sx={{ display:  props.tagging != undefined ? 'none' : 'block' }} >
            <RadioGroup
              row
              name="tagging"
              value={selected}
...

          {
            taggingArr.length === 0 ? null : (
              <Chip
                ...
                disabled={props.tagging != undefined ? true : props.tagDisabled}
